...
async def get_all_tasks_repo() -> dict:
    async with httpx.AsyncClient() as client:
        raw_response = await client.get(BIN_URL, headers=HEADERS)
...
async def reset_full_doc_repo(all_tasks: dict) -> None:
    async with httpx.AsyncClient() as client:
        raw_response = await client.put(BIN_URL, json=all_tasks, headers=HEADERS)
        print(raw_response)


def get_next_id(all_tasks: dict[str, str]) -> str:
    next_id = max(int(k) for k in all_tasks) + 1
    return str(next_id)


# get
async def get_task_by_id_repo(task_id: str) -> SimpleTask:
    all_tasks = await get_all_tasks_repo()
...
# create
async def add_new_task_repo(task: str):
    all_tasks = await get_all_tasks_repo()
    next_id = get_next_id(all_tasks)
    all_tasks[next_id] = task
    await reset_full_doc_repo(all_tasks)


# update
async def update_task_repo(task_id: str, new_task_content: str) -> SimpleTask:
...
# delete
async def delete_task_repo(task_id: str) -> None:
    all_tasks = await get_all_tasks_repo()
